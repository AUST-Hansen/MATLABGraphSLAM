...
%                   icp(q,p,k,twoDee) 
%                       If twoDee is true, only x-y-heading is calculated
%
    ...
    switch arg.Minimize
        case 'point'
            ...
            if (~arg.twoDee)
               ...
               [R,T] = eq_point2D(q(:,q_idx),pt(:,p_idx), weights(p_idx)); 
            end
        case 'plane'
            weights = arg.Weight(match);
...
function [R,T] = eq_point2D(qfull,pfull,weights)

% z-component of the points should be zero
p = pfull(1:2,:);
q = qfull(1:2,:);
m = size(p,2);
n = size(q,2);
...
R2d = V*diag([1 det(U*V')])*transpose(U);
